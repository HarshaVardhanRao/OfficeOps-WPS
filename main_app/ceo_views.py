...
# CEO: Download marks template
@login_required
def ceo_download_marks_template(request, exam_id):
...
# CEO: Upload marks Excel
@csrf_exempt
def ceo_upload_marks(request, exam_id):
    exam = Examination.objects.get(id=exam_id)
    ...
    return render(request, 'ceo_template/ceo_upload_marks.html', {'exam': exam})

@login_required
def ceo_select_exam_upload(request):
    examinations = Examination.objects.all()
    exam_id = request.GET.get('exam_id')
    if exam_id:
        return render(request, 'ceo_template/ceo_select_exam_upload.html', {'examinations': examinations, 'exam_id': exam_id})
    return render(request, 'ceo_template/ceo_select_exam_upload.html', {'examinations': examinations})

@login_required
def ceo_add_examination(request):
    if request.method == 'POST':
        ...
        return render(request, 'ceo_template/ceo_add_examination.html', {
            'standards': Standard.objects.all(),
            'sections': Section.objects.all(),
            'exam_id': exam.id
        })
